 ...
 *   customElements.get(TAG_NAME).registerEffect('miEfecto', class {  // o 'video-showcase' tal cual, si no lo renombraste
 ...
 *
 * SOPORTE YOUTUBE: si `video` / `video-desktop` / `video-mobile` resuelve a
 * una URL de YouTube (embed, watch?v= o youtu.be), el componente cambia
 * internamente a un <iframe> controlado por la YouTube IFrame API (se carga
 * una sola vez, on-demand, sin afectar el modo con archivo mp4). Limitaciones
 * propias del iframe cross-origin:
 *   - El aspect-ratio queda fijo en 16:9 (no hay forma de leer el tamaño real).
 *   - `sound-control` funciona por el ícono (usa la API de YouTube), pero NO
 *     por tocar el video: un click dentro de un iframe de otro origen no es
 *     interceptable desde el documento host.
 *   - REQUIERE servir la página por http(s) (aunque sea un server local).
 *     Abrir el HTML directo por file:// tira "Error de configuración del
 *     reproductor de video": desde fines de 2025 YouTube exige un Referer/
 *     origin válido, y file:// no envía ninguno bajo ninguna configuración.
 *
 * ¿NECESITÁS PROBAR UNA VERSIÓN DRAFT SIN PISAR LA QUE YA ESTÁ EN TU PÁGINA?
 * Cambiar el nombre del archivo y del tag en el HTML NO ALCANZA — el tag
 * queda registrado con el nombre fijado en TAG_NAME, un poco más abajo.
 * Para correr dos copias en paralelo (ej. "video-showcase" y
 * "video-showcase2"), cambiá SOLO esa constante en la copia draft; el resto
 * del archivo no necesita tocarse.
 */
(function () {
  'use strict';
  ...
  /* =========================================================
     0.5 Soporte YouTube (opcional — solo se activa si `video`/
     `video-desktop`/`video-mobile` resuelve a una URL de YouTube)
     ========================================================= */
  ...
  // La YouTube IFrame API es global y asíncrona: se carga una sola vez aunque
  // haya varios <video-showcase> en la página, y se encadena con cualquier
  // window.onYouTubeIframeAPIReady que ya existiera (para no pisarlo).
  ...
  const TEMPLATE =
    ...
    '.yt-player{position:absolute;inset:0;width:100%;height:100%;z-index:1;background:#000;display:none;}' +
    ...
    '<div class="yt-player" part="yt-player"></div>' +
  ...
  class VideoShowcase extends HTMLElement {
    ...
    constructor() {
      ...
      this._els = {
        ...
        ytPlayer: this.shadowRoot.querySelector('.yt-player'),
      ...
      this._ytPlayer = null; // instancia de YT.Player cuando la fuente activa es de YouTube
      this._unmuteRecoveryStep = 0; // ver _toggleSound / onStateChange: recupera la pausa fantasma post-unMute()
      ...
      this._io = new IntersectionObserver(
        (entries) => {
          ...
          if (this._visible) {
            ...
            try { this._ytPlayer.pauseVideo(); } catch (err) { /* no-op */ }
          } else {
    ...
    play() {
      if (this._ytPlayer) return this._ytPlayer.playVideo();
      return this._els.video.play();
    }
    pause() {
      if (this._ytPlayer) { this._ytPlayer.pauseVideo(); return; }
      this._els.video.pause();
    }
    ...
    _updateVideoSource() {
      ...
      this._els.ytPlayer.style.display = 'none';
      if (!src) {
        v.removeAttribute('src');
        return;
      }
    ...
    _setupYouTube(id, src) {
      this._teardownYouTube();
      this._els.ytPlayer.style.display = 'block';
      this._els.ytPlayer.innerHTML = '';
      this._els.stage.setAttribute('data-loading', '');

      // Armamos el <iframe> nosotros (en vez de dejar que YT.Player lo genere)
      // para poder fijar referrerpolicy/allow — YouTube exige un Referer/origin
      // válido desde fines de 2025, algo que un archivo abierto por file://
      // no puede enviar bajo ninguna configuración.
      const isFileOrigin = !location.origin || location.origin === 'null' || location.origin.indexOf('file:') === 0;
      const origin = isFileOrigin ? undefined : location.origin;
      const params = new URLSearchParams({
        autoplay: '1',
        mute: this._soundMuted ? '1' : '0',
        loop: '1',
        playlist: id, // requerido por YouTube para poder loopear un solo video
        playsinline: '1',
        controls: '0',
        rel: '0',
        modestbranding: '1',
      ...
      this._els.ytPlayer.appendChild(iframe);

      loadYouTubeApi().then((YT) => {
        // Puede haberse desconectado o cambiado de fuente mientras cargaba la API.
        if (!this._connected || this._activeSrc !== src) return;
        this._ytPlayer = new YT.Player(iframe, {
          events: {
            onReady: () => {
              this._els.stage.removeAttribute('data-loading');
              if (this._visible) this._maybePlayVideo();
              this._updateSoundIcon();
            },
            onStateChange: (e) => {
              if (e.data === YT.PlayerState.PLAYING) {
                this._unmuteRecoveryStep = 0;
                return;
              }
              if (e.data !== YT.PlayerState.PAUSED || !this._unmuteRecoveryStep) return;
              if (this._unmuteRecoveryStep === 1) {
                this._unmuteRecoveryStep = 2;
                this._ytPlayer.playVideo(); // primer intento: reintentar igual, con sonido
              } else {
                // el navegador insiste en bloquear la reproducción con sonido:
                // priorizamos que el video siga andando antes que quede pausado
                this._unmuteRecoveryStep = 0;
    ...
    _teardownYouTube() {
      if (this._ytPlayer) {
        try { this._ytPlayer.destroy(); } catch (err) { /* no-op */ }
        this._ytPlayer = null;
      }
    }

    _applyDefaultAspect(ytId) {
      // El pedido explícito fue 16:9 fijo para YouTube (no hay forma de leer
      // el tamaño real del video: el iframe es de otro origen).
      this._els.showcase.style.aspectRatio = ytId ? '16 / 9' : this._isMobile ? '9 / 16' : '16 / 9';
    }
    ...
    _maybePlayVideo() {
      if (this._ytPlayer) {
        try { if (this._visible) this._ytPlayer.playVideo(); } catch (err) { /* no-op */ }
        return;
      }
      const v = this._els.video;
    ...
    _toggleSound() {
      ...
      if (this._ytPlayer) {
        if (this._soundMuted) {
          ...
          // Algunos navegadores no reconocen el unMute() disparado vía
          // postMessage como "gesto real del usuario" del lado del iframe,
          // y YouTube responde pausando en vez de seguir reproduciendo con
          // sonido. Armamos una recuperación de hasta 2 pasos (ver
          // onStateChange en _setupYouTube) para no dejarlo trabado en pausa.
          this._unmuteRecoveryStep = 1;
  ...
  if (!customElements.get(TAG_NAME)) {
    customElements.define(TAG_NAME, VideoShowcase);
  }
})();
