 ...
 *   videos              JSON array de URLs.  def: ["./video/prep1.mp4","./video/prep2.mp4"]
 *                       Acepta cualquier cantidad: 1, 2, 3…
 *                       Ej: videos='["./v1.mp4","./v2.mp4","./v3.mp4"]'
 *   orientations        JSON array paralelo a `videos` con "landscape" | "portrait".
 *                       Si se omite o tiene menos elementos que `videos`, el resto
 *                       es "landscape" (default).
 *                       Ej: orientations='["landscape","portrait","landscape"]'
 *   posters             JSON array paralelo a `videos` con URLs de poster (opcional).
 *   captions            JSON array paralelo a `videos` con rótulos (opcional).
 *
 *   ── Aliases de 2 videos (retrocompatibilidad) ──────────────────────────
 *   video1 / video2     Equivalentes a videos[0] / videos[1].
 *   poster1 / poster2   Equivalentes a posters[0] / posters[1].
 *   caption1 / caption2 Equivalentes a captions[0] / captions[1].
 *
...
class PreparativosPena extends HTMLElement {
  ...
  static get observedAttributes() {
    return ['videos', 'orientations', 'posters', 'captions',
            'video1', 'video2', 'poster1', 'poster2', 'bg', 'bg-mobile',
  ...
  // ── Helpers para parsear atributos JSON array de forma segura ─────────────
  _parseJsonAttr(name) {
    try { const v = this.getAttribute(name); return v ? JSON.parse(v) : null; }
    catch { return null; }
  }

  // ── Getters de videos / orientaciones / posters / captions ────────────────
  get _videoList() {
  ...
  get _orientations() {
    const arr = this._parseJsonAttr('orientations') || [];
    const videos = this._videoList;
    return videos.map((_, i) => {
      const o = (arr[i] || 'landscape').toLowerCase();
  ...
  get _captions() {
    const arr = this._parseJsonAttr('captions') || [];
    const videos = this._videoList;
    const legacy = [this.getAttribute('caption1') || '', this.getAttribute('caption2') || ''];
    return videos.map((_, i) => arr[i] ?? legacy[i] ?? '');
  }
  ...
  render() {
    ...
    this.shadowRoot.querySelectorAll('.pp-sound').forEach((btn, i) => {
      btn.addEventListener('click', () => {
        ...
        this._syncSoundButtons(videoEls);
      });
    });
    ...
    return `
      ...
      *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      @media (max-width: 760px) {
        ...
        .pp-videos--count-3,
        .pp-videos--count-4,
        .pp-videos--mixed { grid-template-columns: 1fr; gap: 1rem; }
        .pp-video--landscape { max-height: none; }
        .pp-video--portrait  { max-height: 80vh; }
