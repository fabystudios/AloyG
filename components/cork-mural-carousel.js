 ...
 *   icon-size-mobile  — Alto del ícono (solo cuando es imagen) en mobile  (default: "1.5em")
 *   icon-size-desktop — Alto del ícono (solo cuando es imagen) en desktop (default: "1.5em")
 *                       Acepta cualquier unidad CSS: "32px", "2em", "2.5rem", etc.
...
class CorkMuralCarousel extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const iconSizeMobile  = this.getAttribute('icon-size-mobile')  || '1.5em';
    const iconSizeDesktop = this.getAttribute('icon-size-desktop') || '1.5em';
    // CSS variables consumidas por .cmc-icon-img (ver stylesheet global)
    this.style.setProperty('--cmc-icon-mobile',  iconSizeMobile);
    this.style.setProperty('--cmc-icon-desktop', iconSizeDesktop);
          ...
          ">
            ...
            <div style="text-align:center;margin-bottom:12px;padding:0 20px;">
              <h3 class="cmc-main-title" style="color:${titleColor};font-size:1.3rem;font-weight:700;margin:0;
                ...
                ${_isImgUrl(icon)
                  ? `<img src="${CorkMuralCarousel._esc(icon).replace(/'/g,'%27')}" alt="" class="cmc-icon-img" style="vertical-align:middle;width:auto;display:inline-block;">`
