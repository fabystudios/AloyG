class NoticiaCard extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <article class="card" lang="es" part="card">
        <figure class="media">
  ...
  _toggleExpand() {
    // "Ver más" deshabilitado por completo (mobile y desktop): el texto se
    // corta con el degradé de la última línea, sin expandir. Fue fuente de
    // bugs de interacción difíciles de reproducir/depurar a distancia, y
    // según el largo del texto fallaba también en desktop.
    return;
  }
  ...
  //
  // En DESKTOP: si el texto completo no entra, aparece "Ver más" (expande).
  // En MOBILE: el botón "Ver más" está deshabilitado a propósito (fue fuente
  // de un bug de interacción difícil de reproducir/depurar a distancia). En
  // mobile el texto se corta con el degradé de la última línea y listo — sin
  // botón, sin expandir, sin nada que se pueda quedar "trabado". Si hace
  // falta mostrar más texto en mobile, se maneja con el atributo
  // `texto-mobile` (un texto más corto pensado para que entre solo).
  _setupClamp() {
    ...
    const recompute = () => {
      ...
      // El botón "Ver más" está deshabilitado siempre (ver _toggleExpand):
      // acá solo se decide si hace falta clampear + mostrar el degradé.
      btn.hidden = true;
    ...
    // Ojo: como la card tiene alto fijo (aspect-ratio) y el ancho no
    // cambia por un reflow interno, esto NO dispara solo cuando cambian
    // las métricas de fuente (ver recálculos de respaldo más abajo).
    ...
    // Recálculos de respaldo: el swap de Playfair Display / Inter (Google
    // Fonts) puede cambiar el alto de línea real después de la primera
    // medición, corriendo headline/autor/texto un poco. document.fonts.ready
    // no siempre alcanza a cubrirlo (algunos navegadores lo resuelven antes
    // de que el repintado final esté aplicado), así que se refuerza con
    // varios recálculos escalonados. Es barato: solo mide y, si hace falta,
    // reduce el clamp — nunca empeora nada. El overflow:hidden de .body-wrap
    // además actúa como tope duro por si alguna medición quedara desfasada.
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(() => requestAnimationFrame(recompute));
    }
    [150, 400, 900, 1800].forEach(ms => setTimeout(recompute, ms));
  }

  // Si hay texto-mobile, cambia el contenido del párrafo al cruzar el
  // breakpoint mobile/desktop (ej: al rotar el celular o redimensionar
  // la ventana), y recalcula el clamp con el texto nuevo.
  _setupTextoMobile() {
    if (!this._textoMobile) return;

    const mq = window.matchMedia('(max-width: 560px)');
    const p = this.shadowRoot.querySelector('.body-text');

    const aplicar = (esMobile) => {
      if (this._expanded) return; // no pisar el texto mientras está expandido
      p.textContent = esMobile ? this._textoMobile : this._texto;
      if (this._recompute) requestAnimationFrame(() => requestAnimationFrame(this._recompute));
    };

    const handler = (e) => aplicar(e.matches);
    mq.addEventListener('change', handler);
    this._mqTextoMobile = mq;
    this._mqTextoMobileHandler = handler;
  }
  ...
  _css() {
    return `
      ...
      *{ box-sizing:border-box; }
      ...
      .author__img-link .author__img{ display:block; }
      ...
      .author__name--link:hover{ color: var(--wine); }
      .author__desc{
        font-family: var(--sans);
        font-size:.72rem;
      ...
      .body-wrap{ position:relative; flex:1; min-height:0; overflow:hidden; }
      .body-text{
        margin:0;
        font-family: var(--sans);
        font-size:.92rem;
      ...
      .toggle{
        /* Deshabilitado por completo (mobile y desktop) — ver _toggleExpand. */
