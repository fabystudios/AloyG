 ...
 *  modal-image         Imagen para el modal de zoom (única, para desktop y mobile)
 *  modal-image-desktop Imagen del modal solo para desktop (opcional, ver nota abajo)
 *  modal-image-mobile  Imagen del modal solo para mobile (opcional, ver nota abajo)
 *
 *    Nota: si se pasan modal-image-desktop Y modal-image-mobile, ambas
 *    tienen prioridad sobre modal-image y se usa cada una según el
 *    tamaño de pantalla (breakpoint 769px, igual que poster-desktop/mobile).
 *    Si solo se pasa modal-image (o no se pasa ninguna), el comportamiento
 *    es el mismo que antes: una sola imagen para ambos casos.
...
class FeriaPoster extends HTMLElement {
  ...
  static get observedAttributes() {
    return [
      ...
      'modal-image-desktop', 'modal-image-mobile',
  ...
  get modalImageDesktop() { return this.getAttribute('modal-image-desktop') || ''; }
  get modalImageMobile()  { return this.getAttribute('modal-image-mobile') || ''; }
  get hasDualModalImages() { return !!(this.modalImageDesktop && this.modalImageMobile); }
  ...
  _createModal() {
    ...
    const modalImgMarkup = this.hasDualModalImages
      ? `<picture>
           <source media="(min-width: 769px)" srcset="${this.modalImageDesktop}">
           <img class="fpm-img" src="${this.modalImageMobile}" alt="Feria Parroquial" draggable="false">
         </picture>`
